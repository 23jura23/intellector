...
std::vector<std::shared_ptr<SimpleMove>> Game::allFigureMoves(Position pos,
...
std::optional<PlayerColour> Game::getWinner() const {  // может можно получше
    ...
    if (is_black && is_white) return {};
    if (is_black) return PlayerColour::black_;
    if (is_white) return PlayerColour::white_;
    return {};
}
